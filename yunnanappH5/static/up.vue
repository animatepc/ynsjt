  ...
  mounted() {
    ...
    function _getQueryString(name) {//根据字段看网址是否拼接&字符串
        ...
        if(from || appinstall || sec || timekey){//假如拼接上了
            // window.location.href = `${window.location.href.split('#')[0]}/#/public/newsdetails/1026645531576647680?refType=10&id=1026645531576647680&columnCode=YNSJTSYYNBD`
            ...
            // alert(`${window.location.href.split('#')[0]}#/public/newsdetails/${this.$route.query.id}?refType=${this.$route.query.refType}&id=${this.$route.query.refType}&columnCode=${this.$route.query.columnCode}`)
        }
  },
  watch: {
    path(newVal, oldVal) {
    ...
    getNewsList() {
      ...
      newsApi.getNewsDetails(bodys).then(r => {
        ...
        $.ajax({
          ...
          cache: false, 
          dataType: "json",
          // url: sms+`/weichatShare/sign.do?url=${url}`,
          url: `http://dinglinfeng.4kb.cn/langya/sign.do?url=${url}`,
          cache:false,
          success: function(data){
            // json1.timestamp = data.timestamp;
            // json1.nonceStr = data.nonceStr;
            // json1.signature = data.signature;
