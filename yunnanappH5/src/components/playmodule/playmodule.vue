...
import { sms } from '../../services/commonApi.js'
  ...
  export default {
        ...
        if(n != o){
          this.$nextTick(() => {
            ...
            if (this.imgUrl != "" && this.imgUrl) {
              ...
              $(".audio-box").addClass("audio-block");
              $(".audio-box").removeClass("audio-none");
            } else {
              $(".audio-box").addClass("audio-none");
              $(".audio-box").removeClass("audio-block");
            }
          });
        }
        if(n.isShowChannal){
    ...
    mounted() {
      ...
      var ldBox = $('<div class="clearfix" style="margin-bottom:10px; display:flex;align-items:center;"></div>');
      var ylBox = $('<div class="clearfix" style="margin-bottom:10px; display:flex;align-items:center;"></div>');
      ...
      var ldInput = $('<input class="video-settingBox-ld slider-block" id="video-settingBox-ld" start="aaa" data-mini="true" type="range" min="0" max="' + mrldMax + '" value="' + mrldMax + '">');
      ldInput.on('change input', function () {
        setBrightness($(this).val());
      })
      var ylInput = $('<input class="video-settingBox-ld slider-block" id="video-settingBox-yl" start="aaa" data-mini="true" type="range" min="0" max="' + mrylMax + '" value="' + mrylMax + '">');
      ...
      togglePlayerBox.append(togglePlayerImage)
       ...
       $("#my-player").append(`
        <div class="audio-box perH100 audio-none">
