...
export default {
    ...
    playInfo(n, o) {
      ...
      if (n != o) {
        this.$nextTick(() => {
          ...
          this.imgUrl = this.radioMsg?this.radioMsg.thumbnail ? this.radioMsg.thumbnail : "":'';
          this.imgTitle = this.radioMsg?this.radioMsg.programName:''
            ? this.radioMsg.programName
            : "";
          $(".audio-img").attr({ src: this.imgUrl });
          $(".audio-txt").text(this.imgTitle);
          if (this.imgUrl == "") {
            $(".audio-box").addClass("audio-none");
          } else {
            $(".audio-box").removeClass("audio-none");
          }
        });
      }
      if (n.isShowChannal) {
  ...
  mounted() {
    ...
    function createTable(trList, id) {
      ...
      var table = $(
        '<table data-role="table" id="' +
          (id || "") +
          '" data-mode="columntoggle" class="ui-responsive table-stroke"></table>'
      );
      var tb = $("<tbody></tbody>");
    ...
    this.play = function(url, id) {
      _id = id;
      let type;
      if (!url || url == sms) return;
    ...
    /* 原工具栏按钮 */
    var qxd = $(
      '<button class="vjs-control vjs-button ui-btn-hidden" type="button" aria-live="polite" title="清晰度" aria-disabled="false" data-disabled="false"><span aria-hidden="true" class="vjs-icon-placeholder">标清</button>'
    );
    ...
    var full = $(
      '<button class="vjs-fullscreen-control vjs-control vjs-button ui-btn-hidden"  type="button" aria-live="polite" title="Fullscreen" aria-disabled="false" data-disabled="false"><span aria-hidden="true" class="vjs-icon-placeholder"></span><span class="vjs-control-text">Fullscreen</span></button>'
    );
    ...
    function toggleFull() {
      setTimeout(function() {
        ...
        // 隐藏返回按钮
        let isFull = $("body").is(".full");
        _this.$emit("isFull", isFull);
        ...
        if ($("body").hasClass("full")) {
          ...
          resize();
          // 监听物理返回键
          _this.$emit("changeFull", true);
        } else {
          $(window).off("resize", resize);
          $("#my-player").attr("style", "");
          _this.$emit("changeFull", false);
        }
      }, 100);
    }

    var setting = $(
      '<button class="vjs-control vjs-button ui-btn-hidden" type="button" aria-live="polite" title="设置" aria-disabled="false" data-disabled="false"><span aria-hidden="true" class="vjs-icon-placeholder">设置</button>'
    );
    ...
    var pageBack = $(
      "<span class='pageBack'><image src='static/images//u302.png'></image></span>"
    );
    ...
    var h_50 = $(
      "<div class='hm_btn'><image src='static/images//u394.png'></image>50%</div>"
    );
    var h_75 = $(
      "<div class='hm_btn'><image src='static/images//u394.png'></image>75%</div>"
    );
    var h_100 = $(
      "<div class='hm_btn'><image src='static/images//u394.png'></image>100%</div>"
    );
    var h_tc = $(
      "<div class='hm_btn'><image src='static/images//u394.png'></image>填充</div>"
    );
    ...
    var ldBox = $(
      '<div class="clearfix" style="margin-bottom:10px; display:flex;align-items:center;"></div>'
    );
    var ylBox = $(
      '<div class="clearfix" style="margin-bottom:10px; display:flex;align-items:center;"></div>'
    );
    ...
    var ldInput = $(
      '<input class="video-settingBox-ld slider-block" id="video-settingBox-ld" start="aaa" data-mini="true" type="range" min="0" max="' +
        mrldMax +
        '" value="' +
        mrldMax +
        '">'
    ...
    var ylInput = $(
      '<input class="video-settingBox-ld slider-block" id="video-settingBox-yl" start="aaa" data-mini="true" type="range" min="0" max="' +
        mrylMax +
        '" value="' +
        mrylMax +
        '">'
    ...
    function setLD(n) {
      ...
      if (!isDisShowDilog) {
        showRangeDialog("亮度", parseInt(isAndroid ? ld * 100 / mrldMax : ld));
      }
    ...
    var typeList = $(
      '<div class="typeList" data-role="controlgroup" data-mini="true"></div>'
    );
    var wsList = $(
      '<div class="wsList" data-role="controlgroup" data-mini="wtrue"></div>'
    );
                ...
                data.list.forEach(function(d) {
                  ...
                  w.on(click, function(event) {
                    ...
                    getPDDetail(
                      function(data) {
                        if (data.data.url)
                          _this.play(data.data.url, d.id, "90");
                        setShoucang(
                          data.data.isCollection == "1" ? "取消收藏" : "收藏"
                        );
                      },
                      { id: d.id }
                    );
    ...
    var fx_qq = $(
      "<div class='fx_btn'><image src='static/images//u349.png'></image>QQ</div>"
    );
    ...
    var fx_kj = $(
      "<div class='fx_btn'><image src='static/images//u351.png'></image>QQ空间</div>"
    );
    ...
    var fx_wx = $(
      "<div class='fx_btn'><image src='static/images//u353.png'></image>微信</div>"
    );
    ...
    // })
    var fx_wb = $(
      "<div class='fx_btn'><image src='static/images//u357.png'></image>微博</div>"
    );
    ...
    var fx_fz = $(
      "<div class='fx_btn'><image src='static/images//u347.png'></image>复制链接</div>"
    );
    ...
    var brightnessState = false,
      volumeState = false,
      timeState = false;
    var brightnessStateLen = 0,
      volumeStateLen = 0,
      timeStateLen = 0;
    var stateLen = 10;
    var time_oldx = 0;
    /*亮度滑动*/
    var brightness = $(
      "<div class='video-brightness' ondragstart='return false;'></div>"
    );
    var oldx = 0;
    var downFlag = false,
      volumeFlag = false;
    ...
    brightness.on("touchstart mousedown", function(event) {
      oldx =
        event.originalEvent[pageXY] ||
        event.originalEvent.changedTouches["0"][pageXY];
      time_oldx =
        event.originalEvent[_pageXY] ||
        event.originalEvent.changedTouches["0"][_pageXY];
    ...
    brightness.on("touchmove mousemove", function(event) {
      ...
      if (brightnessState && lrFlag === "l") {
        ...
        if (lrFlag === "l")
          brightnessState = Math.abs((brightnessStateLen += num)) > stateLen;
        if (lrFlag === "r")
          volumeState = Math.abs((volumeStateLen += num)) > stateLen;
    ...
    var mb = myBrowser();
    _this.$bridge.registerhandler("getBrightness", function(
      data,
      responseCallback
    ) {
      if (downFlag) return;
    ...
    _this.$bridge.registerhandler("getVolume", function(
      data,
      responseCallback
    ) {
      if (downFlag) return;
    ...
    $("#my-player").append(`
        <div class="audio-box perH100">
          <span class="audio-txt perW100"></span>
          <img class="audio-img perW100 perH100" src="">
        </div>
      `);
